...
"""
背景
- `reduce-lowering-name-dependence` change 要求 emitter correctness path 不再以 `DecodedInst.name` 作为 lowering authority。

需求/作用
- 静态检查 `sbt/ptx_emit.cpp` 中剩余的 `.name` 读取位置，确保它们只用于显式批准的 comment / diagnostic 场景。

用法
- 直接运行：`python3 tools/check_ptx_emit_name_allowlist.py`

实现原理/处理步骤
- 逐行扫描 `sbt/ptx_emit.cpp` 中的 `.name` 使用。
...
PTX_EMIT = ROOT / "sbt" / "ptx_emit.cpp"
...
def main() -> int:
    lines = PTX_EMIT.read_text(encoding="utf-8").splitlines()
    errors: list[str] = []
    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        if FORBIDDEN_RE.search(line):
            errors.append(f"{PTX_EMIT}:{lineno}: forbidden authority-like name usage: {line.strip()}")
            continue
        if not any(snippet in line for snippet in ALLOWED_SNIPPETS):
            errors.append(f"{PTX_EMIT}:{lineno}: name usage missing allowlist category: {line.strip()}")
