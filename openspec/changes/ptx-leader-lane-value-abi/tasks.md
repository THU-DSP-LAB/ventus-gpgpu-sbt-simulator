...
- [x] 1. 在 `sbt/ptx_emit.cpp` 引入新的 leader-lane 基础语义辅助接口，至少包括：
...
- [x] 2. 建立 leader-lane `x-reg` canonical state 路径：
...
- [x] 3. 实现 structured divergence 的 leader / scalar-state 迁移协议：
  ...
  - 若无法可靠识别 predecessor-edge 放置点或无法放置 shared-join / nested-join 所需协议，显式 fail-fast 而不是继续发射 PTX

- [x] 4. 设计并落地 direct-call value ABI 的 emitter 路径：
...
- [x] 5. 在 leader 合法性边界已经落地后，重构 block 入口 leader derivation 路径：
...
- [x] 6. 实现 all-lane scalar consumer 的统一广播规则：
...
- [x] 7. 处理 staged coexistence 与旧路径退场：
