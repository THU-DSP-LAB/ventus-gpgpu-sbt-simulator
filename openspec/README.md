...
custom 指令与普通 lowering 去名字耦合主题的两步收敛工作现已分别归档到 `reduce-lowering-name-dependence` 与 `unify-cfg-control-semantics`。若后续继续推进 blocked/deferred/research MMA family，应新开 change 承载增量 contract，而不是复用已归档的 control-semantics / lowering-authority change。
