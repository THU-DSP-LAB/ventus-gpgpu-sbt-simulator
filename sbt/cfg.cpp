...
static CfInfo classify(const BundleInst &bi) {
  ...
  if (semantics.is_indirect_terminator || semantics.is_endprg) {
    out.is_terminator = true;
    out.has_fallthrough = false;
    return out;
  }
  if (semantics.is_scalar_branch || semantics.is_vector_branch) {
    out.is_branch = true;
    out.is_terminator = true;
    out.target = static_cast<uint32_t>(semantics.direct_target);
    out.has_fallthrough = true;
    return out;
  }
  if (semantics.is_direct_jump) {
    out.is_jump = true;
    out.is_terminator = true;
    out.target = static_cast<uint32_t>(semantics.direct_target);
    out.has_fallthrough = false;
...
static bool in_range(uint32_t x, uint32_t lo, uint32_t hi_excl) {
...
FunctionCfg build_function_cfg(const std::vector<DecodedInst> &decoded,
                               uint32_t func_start, uint32_t func_end_excl) {
  FunctionCfg cfg;
  ...
  for (const auto &di : decoded) {
    ...
    bi.len =
        static_cast<uint8_t>(di.had_regext ? (di.regext.prefix_bytes + 4) : 4);
    bi.inst = di;
  ...
  for (size_t i = 0; i < cfg.insts.size(); ++i) {
    ...
    require(in_range(bi.pc, func_start, func_end_excl),
            "CFG: instruction pc out of function range");
  ...
  for (size_t i = 0; i < cfg.insts.size(); ++i) {
    ...
    if (semantics.is_join)
      leaders.insert(bi.pc);

    if ((ci.is_branch || ci.is_jump) &&
        in_range(ci.target, func_start, func_end_excl)) {
      require(cfg.inst_index_by_pc.contains(ci.target),
              "CFG: missing branch/jump target at pc=" + hex_u32(ci.target));
  ...
  while (cur < cfg.insts.size()) {
    ...
    for (; cur < cfg.insts.size(); ++cur) {
      ...
      if (ci_prev.is_terminator)
        break;
      if (leaders.contains(cfg.insts[cur].pc))
        break;
    }
  ...
  auto dst_block = [&](uint32_t dst_pc) -> uint32_t {
    ...
    if (it == cfg.inst_pc_to_block.end())
      throw std::runtime_error("CFG: dst pc not in inst_pc_to_block");
    return it->second;
  };
  ...
  for (auto &bb : cfg.blocks) {
    ...
    const auto semantics = sbt::control::classify(last);

    auto add_edge = [&](uint32_t dst_pc, EdgeKind kind) {
      if (!in_range(dst_pc, func_start, func_end_excl))
        return;
      Edge e;
      e.src = bb.start;
    ...
    } else if (ci.is_return || semantics.is_endprg ||
               semantics.is_indirect_terminator) {
      // no succ
    } else {
      add_edge(last.inst_pc + 4, EdgeKind::Fallthrough);
    }
