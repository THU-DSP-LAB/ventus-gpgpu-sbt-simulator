...
# work_group_broadcast builtin PTX lowering 回归
./build/workgroup_broadcast_builtin_test
...
- `sbt/cfg_verify` 的 vector uniform 传播消费共享 `uniform_transfer_kind`，并对 direct call 使用 ELF symbol map 做 call-aware transfer：已知 inlined builtin 应用共享 summary；`work_group_broadcast` 这类会 lower 为 PTX `bar.sync` 的 builtin 还会作为隐式 work-group 同步点参与收敛性检查；ordinary/unresolved/缺 symbol map 的 direct call 按 Ventus ABI 清除 caller-saved `%v0..%v31` 的 vector-uniform facts、保留 callee-saved `%v32..%v255` facts；`sbt_ptx` 对 reachable direct-call callee 传播 call-site 入口 uniform/convergence facts，用于跨函数 barrier 收敛性验证。supported-path 指令缺 metadata 或缺控制流结构化语义时都会直接报错，不再回退到 `_vx/_vi/_vv/_v` suffix 或 mnemonic 猜测。
